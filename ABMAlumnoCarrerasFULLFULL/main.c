...
/*int contarMilanesasCarrera(int idCa, eAlumno alumnos[], int tAlmuno, eAlmuerzo almuerzos[], int tAlmuerzo)
{
    contador = 0;

    for(int i=0; i < tAlumno; i++)
    {
        if(alumnos[i].isEmpty == 0 && alumno[i].idCarrera == idCarrera)
        {
...
void facturacionAlmuerzoAlumno()

float deudaAlumno(eAlumos alumnos[], eAlmuerzo almuerzos[], int tAlm, eComida comidas[], int tCom, int legajo)
{
    float contadorDeuda = 0;

    if(alumnos[i].legajo == legajo && elumnos[i].isEmpty == 0)
    for(int i=0; i < tAl; i++)
    {
        if(almuerzos[i].legajo == legajo)
        {
            for(int j=0; j < tCom; j++)
            {
                if(comidas[j].id == almuerzos[i].idComida)
                {
                    contadorDeuda = contadorDeuda + comidas[j].precio;
                }
            }
        }
    }

    return contadorDeuda;
}

void recaudacionXAlgunaComida(eComida comidas[], int tCom, eAlmuerzo almuerzos[], int tAlm)
{
